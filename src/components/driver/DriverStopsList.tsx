...
import { MapPin, CheckCircle2, Clock, Loader2, Navigation, SkipForward, XCircle } from 'lucide-react';
...
export function DriverStopsList({ stops, routeStatus, onStopCompleted }: DriverStopsListProps) {
  const { mutate: updateStopStatus, isPending, variables } = useUpdateStopStatus();

  const handleStatusChange = (stopId: string, status: 'done' | 'skipped' | 'failed') => {
    updateStopStatus(
      { stopId, status },
          ...
          <div className="p-4 space-y-2">
            {stops.length === 0 ? (
              ...
              stops.map((stop, index) => {
                ...
                const isLoadingThis = isPending && variables?.stopId === stop.id;
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      {/* Status badge for resolved stops */}
                      {isResolved && (
                    ...
                    {isRouteActive && isPendingStop && (
                      <div className="flex gap-2 ml-11">
                        <Button
                          ...
                          onClick={() => handleStatusChange(stop.id, 'done')}
